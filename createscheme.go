...
func main() {
	...
	fmt.Printf("Size of metadata: %d, %d", len(metadataBuffer), cap(metadataBuffer));

	// addr, _, contract, err := DeployGreeter(auth, sim, "DIVA is cool")
	addr, _, contract, err := contracts.DeployIRMAScheme(auth, client, "pbdf", metadataBuffer)
	if err != nil {
	...
	schemeMetadataBuffer, _ := contract.Metadata(nil);
